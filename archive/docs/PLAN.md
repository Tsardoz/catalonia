...
- Positive early-July windows in the `Tmax`, `VPD`, and some deficit-style scans now look more plausibly like the **late-June / early-July transition into pit hardening**, rather than a contradiction of olive RDI practice.
...
### 3. Constructing the olive yield–climate panel is now part of the contribution

The current timing analysis was not possible from any single off-the-shelf dataset.

The workflow had to combine:

- olive yield from the administrative DARP/Gencat spreadsheets
- daily weather from independently sourced AgERA5 climate products
- spatial matching to olive-growing locations / farms
- aggregation back to comparable comarca-year analysis units

This data-construction step is a large part of why this kind of analysis is uncommon in perennial crop work.

### 4. Analysis has moved beyond a single annual lag model
...
- the x-axis is a **fortnight ending date**, not a single-day effect
- so a point labeled `Jul 09` represents roughly **Jun 26–Jul 09**
- the positive early-July windows therefore likely overlap the **onset / early part of pit hardening**
...
The early-July positive windows are now better interpreted as **late-June / early-July onset of pit hardening**, which makes them more consistent with established olive RDI timing than the earlier coarse "fruit set" labeling suggested.
...
- each OLS is still simple: **one climate-window predictor + comarca fixed effects**
- raw p-values do exist, but because the scan tests **many correlated windows**, they should be treated as **exploratory screening statistics**, not strict confirmatory significance tests
...
7. **How should novelty be framed?**
   - likely not as a wholly new method across all crop science
   - better framed as a distinctive application enabled by an unusually labor-intensive olive yield–climate panel

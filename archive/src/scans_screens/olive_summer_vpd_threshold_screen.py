"""Simple summer VPD threshold screen for olive yield using hybrid daily climate."""
...
def build_feature_table(climate: pd.DataFrame) -> pd.DataFrame:
    ...
    for (comarca, year), grp in climate.groupby(["comarca", "year"], sort=True):
        ...
        for thr in THRESHOLDS:
            key = f"vpd_days_ge_{str(thr).replace('.', 'p')}"
            row[key] = int((grp["vpd_mean"] >= thr).sum())
        rows.append(row)
...
def plot_results(results: pd.DataFrame, out_path: Path) -> None:
    ...
    ax.set_title(f"Olive yield vs summer VPD-threshold day count\n{SUMMER_START} to {SUMMER_END}, comarca FE")
...
if __name__ == "__main__":
    ...
    results = []
    for thr in THRESHOLDS:
        col = f"vpd_days_ge_{str(thr).replace('.', 'p')}"
        results.append(fit_single_feature(sample, col))
    results = pd.DataFrame(results).sort_values("threshold_kpa").reset_index(drop=True)

    out_csv = DATA / "olive_summer_vpd_threshold_screen.csv"
    out_fig = FIG / "olive_summer_vpd_threshold_screen.png"

...
from features import phase_for_date_olive
...
def add_phase_bands(ax, sub: pd.DataFrame) -> None:
    ...
    for i in range(1, len(phases) + 1):
        if i == len(phases) or phases[i] != phases[start]:
            ...
            ax.axvspan(x0, x1, color=colors.get(phase, "#eeeeee"), alpha=0.18, zorder=0)
            ax.text((x0 + x1) / 2, 0.97, labels.get(phase, phase.replace("_", " ")), transform=trans,

...
Unit conversions:
  ...
  - Solar radiation flux: J/m²/day → MJ/m²/day  (divide by 1e6)
...
Output CSV columns (one row per comarca-day):
  ...
  srad_mean, srad_min, srad_max         (if solar_radiation_flux downloaded)
"""
...
VARIABLE_MAP = {
    ...
    "solar_radiation_flux":         ("srad",   lambda x: x / 1e6),
}
...
def extract_year(year: int, mask: dict) -> pd.DataFrame:
    ...
    # Build in WARP-specified column order: et0, ea, tmax, tmin, vpd, precip, srad
    OUTPUT_ORDER = ["et0", "ea", "tmax", "tmin", "precip", "srad"]

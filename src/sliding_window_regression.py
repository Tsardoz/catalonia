...
def fit_one(yield_df: pd.DataFrame, feat: pd.DataFrame) -> dict:
    ...
    if df["comarca"].nunique() < 2 or len(df) < 10:
        return {"n": len(df), "coef": np.nan, "stderr": np.nan, "t": np.nan, "p": np.nan, "r2": np.nan}
    dummies = pd.get_dummies(df["comarca"], drop_first=True, dtype=float)
    X = sm.add_constant(pd.concat([df[["feature"]].reset_index(drop=True),
                                    dummies.reset_index(drop=True)], axis=1))
    res = sm.OLS(df["yield_tha"].values, X).fit()
    return {
        "n": len(df),
...
def main() -> None:
    ...
    out = pd.DataFrame(rows)[["window_end_doy", "window_end_date", "n", "coef", "stderr", "t", "p", "r2"]]

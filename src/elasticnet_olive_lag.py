...
def load_climate_biweekly() -> pd.DataFrame:
    """Bin daily climate data into 26 biweekly agronomic-year bins (Dec 1 – Nov 30)."""
...
def plot_irf_single(coef_lag, r2_train, r2_cv, var_label, filename, colour="steelblue"):
    ...
    _add_pheno_bands(ax)
    ax.set_xlabel("Biweekly period of agronomic year (Dec–Nov)")
...
def plot_irf_overlay(coef_vpd, r2cv_vpd, coef_et0, r2cv_et0, filename):
    ...
    ax.set_xlabel("Biweekly period of agronomic year (Dec–Nov)")
    ax.set_ylabel("ElasticNet coefficient (standardised)")
    ax.set_title("Olive yield impulse response — VPD vs ET₀")
...
def plot_ols_comparison(ols_models, lag_col_sets, labels, colours, filename):
    ...
    fig.suptitle("Olive yield — OLS impulse response comparison (4-week bins, comarca FE)",
...
if __name__ == "__main__":
    ...
    print(f"Dataset: {len(df)} rows, {df['comarca'].nunique()} comarcas, "
          f"years {df['agro_year'].min()}–{df['agro_year'].max()}")
    print(f"Biweekly feature columns: {len(vpd_cols)} VPD, {len(et0_cols)} ET₀, "
    ...
    plot_vmax_with_proxy_overlay(
        ...
        precip_proxy,
        "precip_lpf_scaled",
        "olive_ols_vpd_with_precip_overlay.png",
        f"LP-filtered precip proxy (τ={int(LP_TAU_DAYS)} d)",
        "Antecedent rain proxy (scaled 0–1)",
        "Olive yield — Max VPD OLS coefficients with antecedent rain proxy",
        y2_ylim=(0, 1.05),
    )

    et0_proxy = compute_et0_proxy(df)
    plot_vmax_with_proxy_overlay(
        ols_vmax,
        vmax_cols,
        et0_proxy,
        "et0_lpf_scaled",
        "olive_ols_vpd_with_et0_overlay.png",
        f"LP-filtered ET₀ proxy (τ={int(LP_TAU_DAYS)} d)",
        "Atmospheric demand proxy (scaled 0–1)",
        "Olive yield — Max VPD OLS coefficients with ET₀ proxy",
        line_colour="#7d6608",
        fill_colour="#f9e79f",
        y2_ylim=(0, 1.05),
    )

    bucket_proxy = compute_bucket_proxy(df)
    plot_vmax_with_proxy_overlay(
        ols_vmax,
        vmax_cols,
        bucket_proxy,
        "bucket_scaled",
        "olive_ols_vpd_with_bucket_overlay.png",
        f"Bucket water-balance proxy (cap={int(BUCKET_CAPACITY_MM)} mm)",
        "Bucket storage proxy (scaled 0–1)",
        "Olive yield — Max VPD OLS coefficients with bucket water proxy",
        line_colour="#1f618d",
        fill_colour="#aed6f1",
        y2_ylim=(0, 1.05),
    )
    ...
    plot_vmax_with_proxy_overlay(
        ...
        cwb_proxy,
        "cwb_lpf",
        "olive_ols_vpd_with_cwb_overlay.png",
        f"LP-filtered climatic water balance (P - ET₀, τ={int(CWB_TAU_DAYS)} d)",
        "LP-filtered P - ET₀ proxy",
        "Olive yield — Max VPD OLS coefficients with climatic water-balance proxy",
        line_colour="#117864",
        fill_colour="#a9dfbf",
        draw_y2_zero=True,
    ...
    plot_vmax_with_proxy_overlay(
        ...
        cwb_proxy,
        "cwb_lpf",
        "olive_ols_et0_with_cwb_overlay.png",
        f"LP-filtered climatic water balance (P - ET₀, τ={int(CWB_TAU_DAYS)} d)",
        "LP-filtered P - ET₀ proxy",
        "Olive yield — Max ET₀ OLS coefficients with climatic water-balance proxy",
        line_colour="#117864",
        fill_colour="#a9dfbf",
        draw_y2_zero=True,
        guide_bands=RDI_GUIDE_BANDS,
        guide_label="Schematic RDI guide (fruit development / literature)",
        bar_label="Max ET₀ OLS coefficient",
    )
    ...
    plot_vmax_with_proxy_overlay(
        ...
        cwb_proxy,
        "cwb_lpf",
        "olive_ols_precip_with_cwb_overlay.png",
        f"LP-filtered climatic water balance (P - ET₀, τ={int(CWB_TAU_DAYS)} d)",
        "LP-filtered P - ET₀ proxy",
        "Olive yield — Precipitation OLS coefficients with climatic water-balance proxy",
        line_colour="#117864",
        fill_colour="#a9dfbf",
        draw_y2_zero=True,
        guide_bands=RDI_GUIDE_BANDS,
        guide_label="Schematic RDI guide (fruit development / literature)",
        bar_label="Precipitation OLS coefficient",
    )

    # OLS with yield_lag1 control — robustness check
    df_lag = df.dropna(subset=["yield_lag1"]).reset_index(drop=True)
    print(f"\n{'#'*60}")
    print(f"OLS WITH LAGGED YIELD (yield_t-1) — {len(df_lag)} rows")
    print("#"*60)
    ols_vmax_lag = fit_ols(df_lag, vmax_cols + ["yield_lag1"], "Max VPD + yield_lag1")
    ols_emax_lag = fit_ols(df_lag, emax_cols + ["yield_lag1"], "Max ET\u2080 + yield_lag1")
    ols_precip_lag = fit_ols(df_lag, precip_cols + ["yield_lag1"], "Precipitation + yield_lag1")
